module ShowParser ( parserShow ) where
...
parserShow = \str -> xml_header++(run_parser showParser str)


run_parser :: Parser a -> String -> a
run_parser p str =
  case parse p "" str of
    Left err -> error $ "Parser error at" ++ (show err)
    Right val -> val
...
tuple_parser = do
  ...
  return $ tag "tuple" $ joinNL $ map (tag "tuple-elt") ls
